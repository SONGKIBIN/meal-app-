  ...
  sections: [
    {
      ...
      children: [
        ...
        h2("통근버스 신청·관리 (신규, 비공개 개발자 모드)"),
        ...bullets([
          "이 기능은 완성 전까지 비공개 상태이며, 마스터 관리자(사번 admin) 계정과 마스터가 권한을 부여한 인원만 미리 사용할 수 있습니다. 마스터 관리자 계정으로 로그인할 때는 비밀번호를 추가로 입력해야 합니다(최초 비밀번호는 MASTER_INITIAL_PASSWORD 환경변수, 기본값 admin1234!). 로그인 후 [관리자 → 개발자 모드]에서 비밀번호를 원하는 값으로 꼭 바꿔주세요.",
          "마스터 관리자가 [관리자 → 개발자 모드]에서 '공개로 전환'을 눌러야 비로소 전 직원 화면에 '통근버스' 탭이 나타납니다. 이 '개발자 모드' 메뉴는 일반 관리자(role=admin) 화면에는 아예 보이지 않습니다.",
          "공개되면 직원들은 '통근버스' 탭에서 평택버스(평택·공도·내리)/공도버스(공도·내리)/안성버스(안성시내) 중 원하는 코스와 출근운행/정시퇴근운행/연장퇴근운행, 정류장을 선택해 승차 신청·취소를 할 수 있습니다(코스는 관리자가 자유롭게 추가/수정 가능). 도급(단체) 계정은 식수 신청과 동일하게 인원수를 직접 입력해 이용합니다.",
          "평일(월~금, 공휴일 제외)은 기본적으로 운행하는 것으로 간주하고, 특근일·대체휴무일·토요일·일요일 등은 담당 관리자가 [통근차량 관리 → 운행일 지정]에서 차량별로 직접 켜줘야 운행됩니다.",
          "마스터 관리자는 [관리자 → 개발자 모드]에서 직원별로 '통근 차량 관리 관리자'와 '기사' 권한을 부여할 수 있습니다 (식수신청 관리자·부서 운영자 권한은 기존과 동일하게 [직원 관리]에서 부여합니다).",
          "'통근 차량 관리 관리자' 권한을 받은 직원은 [통근차량 관리] 화면에서 차량/코스 관리, 날짜별 운행 지정, 신청 현황(취소 대신 처리 포함), 일일·월간 인원 집계 확인 및 엑셀 다운로드·인쇄를 할 수 있습니다. 담당 차량을 지정하면 그 차량만, 비워두면 전체 차량을 관리합니다.",
          "'기사' 권한을 받은 직원은 [기사 모드] 화면에서 본인이 배정된 차량의 오늘(또는 지정한 날짜) 운행 여부와 신청 인원을 확인하고, 실제 운행 여부·탑승 인원·메모를 기록할 수 있습니다. 다른 차량의 운행 현황도 참고용으로 함께 볼 수 있습니다(읽기 전용).",
          "SMTP 정보(발신 메일 계정)를 환경변수에 등록하면, 매일 08:00에 전날 통근버스 운행일지가 마스터 관리자와 통근 차량 관리 관리자 중 이메일을 등록한 사람에게 자동으로 발송됩니다. 등록하지 않으면 이메일 기능만 비활성화되고 나머지 기능은 동일하게 동작합니다.",
          "식수신청 관리자도 본인 이메일을 등록하면, 기존 웹 푸시 알림과 별개로 매일 중식/석식 신청 집계를 이메일로도 받아볼 수 있습니다.",
        ]),
