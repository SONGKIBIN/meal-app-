...
async function computePendingSummary(date) {
  ...
  const rows = await Reservation.find({ date, $or: [{ status: "applied" }, { declinedHeadcount: { $gt: 0 } }] }).lean();
  ...
  // "미신청(미정)" 명단: 신청도, 안 먹어요 등록도 하지 않은 사람만 포함합니다.
  ...
  function build(appliedMap, declinedMap) {
    ...
    for (const e of employees) {
      ...
      if (e.employeeType === "contractor" && Number.isInteger(e.totalHeadcount) && e.totalHeadcount > 0) {
        ...
        const shortfall = e.totalHeadcount - applied - declinedHeadcount;
        if (shortfall > 0) {
  ...
  // "안 먹어요" 명단: 명시적으로 안 먹는다고 등록한 사람 목록입니다.
...
router.put("/reservations/override", async (req, res) => {
  try {
    ...
    // 개인 직원을 관리자가 강제로 "신청" 처리하면, 그 사람이 이전에 등록해둔 "안 먹어요" 표시와
...
router.get("/summary/monthly", async (req, res) => {
  ...
  const declinedRows = await Reservation.find({ date: { $in: dates }, declinedHeadcount: { $gt: 0 } }).lean();
...
// 한 달 치 부서별 집계 + 개별 평가(사유 포함) 목록도 함께 제공합니다 (일별 화면과 동일하게, 클릭 시 등록 일시 확인 가능).
router.get("/ratings/summary/monthly", async (req, res) => {
  const { month } = req.query;
  if (!MONTH_RE.test(month || "")) return res.status(400).json({ error: "month 파라미터가 필요합니다 (YYYY-MM)." });
  const [year, mon] = month.split("-").map(Number);
  const dates = getMonthDates(year, mon);
  const ratings = await MealRating.find({ date: { $in: dates } }).sort({ date: 1, department: 1, employeeName: 1 }).lean();
  res.json({
    month,
    ratings,
    byDepartment: aggregateRatingsByDepartment(ratings),
...
router.get("/export/daily", async (req, res) => {
  try {
    ...
    // "안 먹어요" 명단 시트
    const declinedRows = await Reservation.find({ date, declinedHeadcount: { $gt: 0 } })
...
router.get("/export/monthly", async (req, res) => {
  try {
    ...
    // "안 먹어요" 명단 시트 (이번 달)
    const declinedRows = await Reservation.find({ date: { $in: dates }, declinedHeadcount: { $gt: 0 } })
