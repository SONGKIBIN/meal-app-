...
/* ---------------------------- 날짜별 운행 여부 지정 ---------------------------- */
// 평일은 기본 운행, 특근일/대체휴무일/토요일/일요일 등은 여기서 켜줘야만 운행됩니다 (평일도 명시적으로 끌 수 있음).
...
// 지정을 지우고 기본값(평일 운행/휴일 미운행)으로 되돌립니다.
...
// 관리자가 특정 직원의 승차 신청을 대신 취소합니다 (담당 차량 범위 내에서만 가능).
router.delete("/ride", async (req, res) => {
  try {
    const { rideId } = req.body;
    if (!rideId) return res.status(400).json({ error: "잘못된 요청입니다." });
...
async function dailyRiders(req, date) {
  ...
  const rides = await BusRide.find({ date, status: "applied", vehicleId: { $in: vehicleIds } })
    .sort({ tripType: 1, department: 1, employeeName: 1 })
    .lean();
  const byVehicleTrip = new Map();
  for (const r of rides) {
    const key = `${String(r.vehicleId)}_${r.tripType}`;
    if (!byVehicleTrip.has(key)) byVehicleTrip.set(key, { count: 0, riders: [] });
    const entry = byVehicleTrip.get(key);
    entry.count += r.headcount || 1;
    entry.riders.push(r);
  }
  const vehicleSummaries = vehicles.map((v) => ({
    ...
    trips: TRIP_TYPES.map((tripType) => {
      const entry = byVehicleTrip.get(`${String(v._id)}_${tripType}`) || { count: 0, riders: [] };
      return { tripType, label: TRIP_LABEL[tripType], count: entry.count, riders: entry.riders };
    }),
  }));
  return { vehicles: vehicleSummaries, rides };
}
...
router.get("/summary/monthly", async (req, res) => {
  try {
    ...
    const vehicles = await scopedVehicles(req, { active: true });
    const vehicleIds = vehicles.map((v) => v._id);
    const rides = await BusRide.find({ date: { $in: dates }, status: "applied", vehicleId: { $in: vehicleIds } }).lean();

    const byDate = {};
    for (const d of dates) byDate[d] = { date: d, commute: 0, regularLeave: 0, extendedLeave: 0 };
    for (const r of rides) {
      byDate[r.date][r.tripType] += r.headcount || 1;
    }
...
router.get("/export/monthly", async (req, res) => {
  try {
    ...
    const vehicles = await scopedVehicles(req, { active: true });
    const vehicleIds = vehicles.map((v) => v._id);
    const rides = await BusRide.find({ date: { $in: dates }, status: "applied", vehicleId: { $in: vehicleIds } })
      .sort({ date: 1 })
      .lean();
    ...
    const byDate = {};
    for (const d of dates) byDate[d] = { date: d, commute: 0, regularLeave: 0, extendedLeave: 0 };
    for (const r of rides) byDate[r.date][r.tripType] += r.headcount || 1;
