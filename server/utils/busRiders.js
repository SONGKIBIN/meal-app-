...
// - 기본 운행일(평일, 공휴일 아님)에는 BusDefaultRide(기본 등록)를 자동으로 탑승자에 포함하되,
//   그날 명시적으로 취소(BusRide status=cancelled)했거나 다른 차량으로 명시 신청(status=applied)한
//   사람은 기본 등록에서 제외합니다(아래에서 명시 신청 쪽으로 다시 포함됨).
...
// allDefaults: BusDefaultRide 문서 배열 (날짜와 무관, 전체 또는 관심 차량 범위)
